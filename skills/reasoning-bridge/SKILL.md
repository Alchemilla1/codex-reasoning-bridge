...
If the user says `preview`, `draft`, `prepare only`, or otherwise clearly asks
not to send, write the complete request and stop before submitting it.
...
Write as an engineer briefing a senior collaborator. Explain the project, the
decision, the evidence, the failed or rejected approaches, the remaining
options, and the user's priorities in natural paragraphs. Include short code or
error excerpts only when they help the advisor reason independently.
